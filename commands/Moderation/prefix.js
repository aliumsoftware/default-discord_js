...
const db = require('quick.db');
  ...
  let check = client.emojis.get(':white_check_mark:');
    ...
    let embed = new RichEmbed()
      .setColor(red)
      .setDescription(`You don't have the vaild permissions to use this command!`)
  if(!message.member.hasPermission(['ADMINISTRATOR', 'MANAGE_GUILD']) || !message.guild.owner) return message.channel.send(embed);
  if(!args[0]) {
    const e = new RichEmbed()
      .setColor(red)
      .setDescription(`You need to supply and argument for me to change the prefix too.`)
